...
class WatchListController extends Controller
{
    ...
    public function addToWatchList(Request $request){
        ...
        // dd($valid);
        if(count($valid)!=0){
            $movie = DB::table('watch_lists')->join('movies','movies.id','=','watch_lists.movie_id')->paginate(4);
        }else{
            WatchList::create([
    ...
    public function changeStatus(Request $request, $wl_id){
        ...
        $movie = DB::table('movies')->join('watch_lists','movies.id','=','watch_lists.movie_id')->paginate(4);
